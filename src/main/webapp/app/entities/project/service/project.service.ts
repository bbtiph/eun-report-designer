...
type RestOf<T extends IProject | NewProject> = Omit<T, 'sysCreatTimestamp' | 'sysModifTimestamp'> & {
...
export class ProjectService {
  ...
  protected convertDateFromClient<T extends IProject | NewProject | PartialUpdateProject>(project: T): RestOf<T> {
    return {
      ...project,
      sysCreatTimestamp: project.sysCreatTimestamp?.toJSON() ?? null,
      sysModifTimestamp: project.sysModifTimestamp?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restProject: RestProject): IProject {
    return {
      ...restProject,
      sysCreatTimestamp: restProject.sysCreatTimestamp ? dayjs(restProject.sysCreatTimestamp) : undefined,
      sysModifTimestamp: restProject.sysModifTimestamp ? dayjs(restProject.sysModifTimestamp) : undefined,
    };
  }
