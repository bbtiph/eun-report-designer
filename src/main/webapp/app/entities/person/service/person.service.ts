...
type RestOf<T extends IPerson | NewPerson> = Omit<T, 'lastLoginDate'> & {
...
export class PersonService {
  ...
  protected convertDateFromClient<T extends IPerson | NewPerson | PartialUpdatePerson>(person: T): RestOf<T> {
    return {
      ...person,
      lastLoginDate: person.lastLoginDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPerson: RestPerson): IPerson {
    return {
      ...restPerson,
      lastLoginDate: restPerson.lastLoginDate ? dayjs(restPerson.lastLoginDate) : undefined,
    };
  }
